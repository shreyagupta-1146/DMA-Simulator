...
function initializeEventListeners() {
    ...
    // START BUTTON - FIXED WITH GLOBAL SOCKET
document.getElementById('start-transfer').addEventListener('click', () => {
    ...
    const fileType = document.getElementById('file-type-select').value || 'text';
    const simSpeed = parseFloat(document.getElementById('sim-speed').value) || 1.0;
    
    console.log('📤 Emitting start_transfer event...');
    
    window.globalSocket.emit('start_transfer', {  // FIXED: Using globalSocket
...
function stopSimulation() {
    ...
    handleSimulationStop();
    addLogEntry('Simulation stopped by user', 'warning');
}
